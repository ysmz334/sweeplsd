...
Akinlar & Topal の ED_Lib EDLines）を同一ISAターゲットでビルドしたものです。

| | SweepLSD (one-pass) | ELSED | EDLines (ED_Lib) | LSD |
|---|---|---|---|---|
| 1枚あたり中央値時間 | **約11 ms** | 約28 ms | 約39 ms | 約230 ms |
...
完全にベクトル化します（FullHD で one-pass 約11ms・multi-pass 約16ms）。MSVC は
ビルド・全テスト通過しますが、現状バイトカーネルをベクトル化しないため約39ms（約3.4倍遅）です。Windows で性能が必要な
