...
**コンパイラ性能の注記 — GCC 15 以降でビルドしてください。** カーネルは設計方針として
SIMD イントリンシクスを含まない（FPGA志向・可読性重視）ため、速度は**すべて**コンパイラの
自動ベクトル化に依存します。その差は4倍に達します。検出結果は変わりません——下記すべての
ツールチェーンで出力は bit-identical です（GCC / Clang / MSVC 間を float 単位で照合、
GCC 世代間は186枚、いずれも両ドライバで検証済み）。

| ツールチェーン | one-pass | multi-pass | GCC 15.2 比 |
|---|---|---|---|
| **GCC 15.2 / 16.1** | **約8.8 ms** | 約12 ms | 1.00× |
| GCC 8.1（2018年） | 約11.4 ms | 約15 ms | 1.25× |
| Clang 22.1（llvm-mingw） | 約33 ms | 約34 ms | 3.8× |
| MSVC 19.4x | 約40 ms | 約42 ms | 4.6× |

Clang / MSVC の差は**たった1つのカーネル**から来ています。端点候補の 5×5 リング判定が
GCC 15.2 で 2.6 ms のところ Clang 27.8 ms・MSVC 26.6 ms となり、平滑化とラベリングは
どのコンパイラでも誤差範囲です（MSVC はさらに勾配で3倍・エッジで2倍を失います）。
ISA フラグの問題ではありません（`-march=native` でも変わらず）。これはコンパイラの
優劣ではなく**カーネルの書き方の移植性の限界**で、解消は今後の課題です
（[`docs/benchmarks.html`](https://ysmz334.github.io/sweeplsd/benchmarks.html) §6 参照）。
Clang・MSVC ともビルドは通り、全テストに通り、出力も完全に同一です——約4倍遅いだけです。
